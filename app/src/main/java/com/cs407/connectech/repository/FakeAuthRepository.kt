...
class FakeAuthRepository : AuthRepository {
     ...
     */
    override suspend fun register(email: String, password: String): LiveData<Result<User>> {
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                if (!email.contains("@")) {
                    result.postValue(Result.failure(Exception("Email must contain '@'")))
                } else if (password.length < 6) {
                    result.postValue(Result.failure(Exception("Password must be at least 6 characters long")))
                } else {
                    val user = User(
